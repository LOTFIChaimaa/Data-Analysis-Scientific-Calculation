...
%% Utilisation de l'ACP pour detecter plusieurs variables.
...
% principaux.
% On utilise la matrice transposée de X pour pouvoir détecter
% les classes de variables et non d'individus.
 X = transpose(X);
 n = size(X,1);
 X_Moyenne = mean(X);
...
C = X_Centree * W;
figure(2),
plot3(C(:,1),C(:,2),C(:,3),'r*');grid on
title('Proj. des donnees sur les 3 premiers axes ppaux')

%Commentaire : On constate qu'il y a 6 classes de variables.
